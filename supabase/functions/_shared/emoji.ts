// Slack-style emoji shortcodes <-> Unicode, generated from iamcal/emoji-data
// (the dataset Slack itself uses; 1972 names). Base emoji only — skin-tone
// suffixes (`name::skin-tone-N`) are stripped by the lookup helpers.
// Primary short_names come before aliases, so the reverse map (first write
// wins) returns the primary name.
//
// Regenerate: fetch https://raw.githubusercontent.com/iamcal/emoji-data/master/emoji.json
// and rebuild the record from short_names/unified (see repo history).

const EMOJI_BY_SHORTCODE: Record<string, string> = {
  "hash": "#\ufe0f\u20e3",
  "keycap_star": "*\ufe0f\u20e3",
  "zero": "0\ufe0f\u20e3",
  "one": "1\ufe0f\u20e3",
...
const SHORTCODE_BY_EMOJI: Record<string, string> = {};
for (const [name, char] of Object.entries(EMOJI_BY_SHORTCODE)) {
  if (!(char in SHORTCODE_BY_EMOJI)) SHORTCODE_BY_EMOJI[char] = name;
}

/** ":thumbsup:", "thumbsup" or "thumbsup::skin-tone-3" → "👍" */
export function emojiFromShortcode(name: string): string | undefined {
  // Split the skin-tone suffix off before stripping wrapper colons — the
  // other way round would mangle "thumbsup::skin-tone-3".
  const base = name.split("::")[0].replaceAll(":", "");
  return EMOJI_BY_SHORTCODE[base];
}

/** "👍" → "thumbsup" (primary short name). Tries the raw char, then
 * without the FE0F variation selector, which reaction payloads often omit
 * or add inconsistently across services. */
export function shortcodeFromEmoji(emoji: string): string | undefined {
  return SHORTCODE_BY_EMOJI[emoji] ??
    SHORTCODE_BY_EMOJI[emoji.replaceAll("\uFE0F", "")] ??
    SHORTCODE_BY_EMOJI[emoji + "\uFE0F"];
}
